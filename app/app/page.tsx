...
import {
  ...
  MAPBIOMAS_URL,
  Chapter,
} from '@/components/presentation/editorial';
import { MobileNavigation } from '@/components/presentation/mobile-navigation';
...
function sourceAbsenceCopy(
  source: 'census' | 'transfers',
...
function ThirtySecondSummary({
  ...
  const atlasPeriod = metadata
    ? `${metadata.sources.atlas.first_year} a ${metadata.sources.atlas.latest_year}`
    : 'período não informado';
  const mapBiomasPeriod =
    firstYear && latestYear ? `${firstYear} a ${latestYear}` : null;

  return (
    <section
      aria-labelledby="resumo-30-segundos"
      className="summary-sheet mb-7 rounded-2xl border border-border bg-card p-5 sm:p-8"
    >
      ...
      <div className="pt-5">
        <p className="max-w-4xl text-base leading-7 text-muted-foreground sm:text-lg">
          {summary.thirty_second_text.replace(
            /\b(\d+)\.(\d{2})(?= km²)/g,
            '$1,$2',
          )}
        </p>
        <dl className="summary-metrics mt-7" data-territory={hasMapBiomas}>
          <div>
            <dt>Registros relacionados à chuva</dt>
            <dd className="text-primary" data-absence={!hasAtlas}>
              {hasAtlas
          ...
          {hasMapBiomas && (
            <>
              <div>
                <dt>Área urbanizada</dt>
                <dd className="text-rain-strong">
                  {formatSignedPercentage(change?.urban_area_change_pct) ??
                    'Não publicada'}
                </dd>
                {mapBiomasPeriod && (
              ...
              <div>
                <dt>Vegetação nativa</dt>
                <dd className="text-[var(--chart-vegetation)]">
      ...
      <div className="source-note">
        <span>
          {sourceLink(ATLAS_URL, 'Atlas/S2ID')}: {atlasPeriod}
        </span>
        <span>
          {sourceLink(MAPBIOMAS_URL, 'MapBiomas')}:{' '}
          {mapBiomasPeriod ?? 'sem cobertura publicada para este município'}
        </span>
      </div>
    </section>
  );
    ...
    <Card className="elevated-card h-full border-0 bg-card shadow-[0_12px_45px_rgb(21_42_57/8%)] ring-border">
        ...
        <CardContent className="py-8">
          <div className="rounded-xl border border-dashed border-border bg-muted/40 p-6">
            ...
            <p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">
              ...
              recorte publicado. Isso não significa ausência de evento, risco ou
      ...
      <CardFooter className="flex-col items-start gap-2 text-xs leading-5 text-muted-foreground">
        ...
        {hasRecord && (
          <p className="w-full">
            <strong>
              {numberFormatter.format(history.recognized_event_count)}
            </strong>{' '}
...
function CensusCard({
  ...
  return (
    <Card className="elevated-card h-full border-0 bg-card shadow-sm ring-border">
      <CardHeader>
        <div className="mb-3 grid size-11 place-items-center rounded-xl bg-rain-soft text-rain-strong">
      ...
      <CardContent className="flex-1 pt-2">
        {!hasRecord ? (
          ...
          <>
            <div className="font-sans text-4xl font-bold tracking-tight text-primary tabular-nums">
            ...
            <div className="mt-6" role="group" aria-label="Parcela observada">
              ...
              <div
                ...
                className="h-2 overflow-hidden rounded-full bg-rain-soft"
                aria-label="Parcela de domicílios fora da categoria selecionada"
                role="progressbar"
              >
                <span
                  className="block h-full rounded-full bg-rain-strong"
        ...
        <p className="mt-6 flex gap-2 border-t border-border pt-5 text-sm leading-6 text-muted-foreground">
          <Info aria-hidden="true" className="mt-0.5 size-4 shrink-0" />
...
export function LandCoverCard({
  ...
  const history = [...landCover.history].sort(
  ...
  const nativeVegetationPercentageChange = change?.native_vegetation_change_pct;
...
export default function Home() {
  ...
  const [selected, setSelected] = useState<MunicipalityPresentation | null>(
    null,
  );
  ...
  function loadMunicipality(entry: MunicipalIndexEntry) {
    ...
    setQuery(`${entry.municipio}, ${entry.uf}`);
    setSelected(null);
  ...
  const results = useMemo(() => {
    ...
    const isStateQuery =
      ...
      searchEntries.some(
        ({ municipality }) => municipality.uf.toLowerCase() === normalizedQuery,
      );
    return searchEntries
      .filter(({ municipality, search }) =>
        isStateQuery
          ? municipality.uf.toLowerCase() === normalizedQuery
          : search.includes(normalizedQuery),
      )
      .slice(0, 50)
  ...
  useEffect(() => {
    ...
    if (searchFocused && active) {
      document
        .getElementById(`municipio-option-${active.codigo_ibge}`)
        ?.scrollIntoView({
          block: 'nearest',
          inline: 'nearest',
          behavior: 'instant',
  ...
  }, [activeResultIndex, results, searchFocused]);

  function chooseMunicipality(municipality: MunicipalIndexEntry) {
    searchInputRef.current?.blur();
    setSearchFocused(false);
    setQuery(`${municipality.municipio}, ${municipality.uf}`);
  ...
  function submitSearch() {
    const exact = municipalities.filter((entry) =>
      [
        entry.codigo_ibge,
        entry.municipio,
        `${entry.municipio}, ${entry.uf}`,
      ].some(
        (value) => normalizeSearch(value) === normalizeSearch(query.trim()),
      ),
    );
    const target =
      activeResultIndex >= 0 ? results[activeResultIndex] : undefined;
    if (target || exact.length === 1 || results.length === 1) {
      chooseMunicipality(target ?? exact[0] ?? results[0]);
    } else {
  ...
  return (
    <>
      ...
      <main
        className="page-ambient min-h-screen bg-background text-foreground"
        id="topo"
      >
        <div className="border-b border-white/10 bg-primary text-primary-foreground">
          <div className="mx-auto flex min-h-11 max-w-7xl items-center justify-between gap-4 px-4 py-2 text-sm font-bold text-white/85 sm:px-8">
            <span>Conhecer o território é o primeiro passo.</span>
        ...
        <header className="editorial-header border-b border-border/80 bg-background/95 backdrop-blur-xl">
          <div className="mx-auto flex min-h-20 max-w-7xl items-center justify-between gap-3 px-4 py-3 sm:px-8">
            <a className="group flex items-center gap-3" href="#topo">
              ...
              <span>
                <span className="font-brand block text-[1.25rem] font-bold leading-none tracking-[-0.025em] sm:text-[1.45rem]">
                  Antes da Chuva
                </span>
                <span className="mt-1.5 block text-sm font-bold text-muted-foreground">
                  Dados para agir antes
                </span>
              </span>
            </a>
            <nav
        ...
        >
          ...
          <div className="relative mx-auto grid max-w-7xl gap-8 px-4 py-8 sm:px-8 sm:py-12 lg:grid-cols-12 lg:items-center lg:gap-14">
            <div className="min-w-0 text-white lg:col-span-7">
              <p className="hero-eyebrow mb-5">Um olhar sobre sua cidade</p>
              <h1 className="hero-title font-heading font-semibold">
                Antes da próxima chuva, <span>conheça sua cidade.</span>
              </h1>
              <p className="mt-5 max-w-xl text-base leading-7 text-white/85 sm:text-lg">
                Veja o histórico de desastres, as mudanças no território e as
                evidências de preparação do seu município.
              </p>
              ...
              <div className="relative mt-8 min-w-0 rounded-2xl border border-white/15 bg-white p-3 text-foreground shadow-[0_24px_80px_rgb(4_20_32/28%)] sm:p-4">
                ...
                {indexError ? (
                  <p
                    className="px-2 pt-2 text-xs font-bold text-destructive"
                    role="alert"
                  >
                    {indexError}
                  </p>
                  ...
                  <p
                    className="px-2 pt-2 text-xs font-bold text-amber-900"
                    role="alert"
                  >
                    {metadataError}
                  </p>
                  ...
                  <p className="px-2 pt-2 text-xs text-muted-foreground">
                    Escolha uma sugestão ou use as setas e Enter.
                  </p>
                )}
              </div>
              <div className="hero-proof">
                <span>
                  ...
                  {metadata
                    ? `${numberFormatter.format(metadata.territorial_universe.municipality_count)} localidades`
                    : 'Dados públicos municipais'}
                </span>
                <span>
                  <FileSearch className="size-4" aria-hidden="true" />
                  Fontes e limites à vista
                </span>
              </div>
              {municipalities.some(
                (entry) => entry.codigo_ibge === '4202404',
              ) && (
                <button
                  className="mt-4 inline-flex min-h-11 items-center gap-2 text-sm font-bold text-white underline decoration-white/40 underline-offset-4 hover:decoration-white"
                  type="button"
                  onClick={() => {
                    const example = municipalities.find(
                      (entry) => entry.codigo_ibge === '4202404',
                    );
                    if (example) chooseMunicipality(example);
                  }}
                >
                  Conhecer Blumenau como exemplo{' '}
                  <ArrowRight className="size-4" aria-hidden="true" />
                </button>
              )}
            </div>

            <figure className="hero-map-frame relative min-h-[200px] overflow-hidden border border-white/20 bg-white/10 sm:min-h-[300px] lg:col-span-5 lg:min-h-[520px]">
              <Image
                ...
                fill
                priority
                sizes="(max-width: 1023px) 100vw, (max-width: 1280px) 40vw, 480px"
                src="/hero-map.webp"
              />
              <figcaption className="absolute right-4 bottom-4 left-4 rounded-xl border border-white/20 bg-primary/95 p-4 text-white sm:p-5">
                <p className="text-sm font-bold text-white/80">
                  O território conta uma história.
                </p>
                <p className="mt-2 max-w-sm font-heading text-lg font-semibold leading-snug sm:text-xl">
                  Os dados ajudam a fazer as próximas perguntas.
                </p>
        ...
        <section
          className="relative z-0 mx-auto max-w-7xl px-4 py-8 sm:px-8 sm:py-12"
          id="resultado"
          tabIndex={-1}
        >
          <p className="sr-only" role="status">
            {payloadLoading
              ? 'Carregando dados municipais'
              : selected
                ? `Leitura de ${selected.municipality.municipio} carregada`
                : ''}
          </p>
          {indexLoading ? (
            <div className="rounded-2xl border border-dashed border-border bg-card/70 p-8 text-center">
              ...
              <p className="mt-2 text-sm text-muted-foreground">
                A busca será habilitada assim que a lista publicada estiver
                pronta.
              </p>
            </div>
          ) : indexError ? (
            ...
            >
              ...
              <p className="mt-2 text-sm leading-6 text-muted-foreground">
                {requestedCodeError} Use a busca para escolher um município
                válido.
              </p>
            </div>
            ...
            <div className="rounded-2xl border border-dashed border-border bg-card/70 p-8 text-center">
              ...
              />
              <h2 className="font-heading text-2xl font-semibold">
                Carregando dados de {selectedEntry.municipio},{' '}
                {selectedEntry.uf}
              </h2>
              <p className="mt-2 text-sm text-muted-foreground">
            ...
            <>
              <div className="mb-4 flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
                <div>
                  <div className="mb-2 flex items-center gap-2 text-sm font-bold text-primary">
                    ...
                    {selected.municipality.municipio} ·{' '}
                    {selected.municipality.uf}
                  </div>
                  <p className="text-sm text-muted-foreground">
                    Sua leitura municipal
                  </p>
                </div>
                <div className="flex flex-wrap items-center gap-2 text-sm text-muted-foreground">
                  ...
                  <a
                    className="inline-flex min-h-11 items-center gap-2 font-bold text-primary underline underline-offset-4"
              ...
              <nav
                className="municipality-nav"
                aria-label="Seções da leitura municipal"
              >
                <a href="#resultado">Resumo</a>
                <a href="#capitulo-historico">Histórico</a>
                <a href="#capitulo-territorio">Território</a>
                <a href="#capitulo-preparacao">Preparação</a>
                <a href="#alertas">Receber alertas</a>
              </nav>
              ...
              <section id="capitulo-historico">
                <Chapter number="01" title="A memória da chuva">
                  O que os registros revelam sobre os eventos no município.
                </Chapter>
                <div className="space-y-5">
                  <DisasterHistory
                    key={selected.municipality.codigo_ibge}
                    disasters={selected.disasters}
                  />
              ...
              <section id="capitulo-territorio">
                <Chapter number="02" title="Um território em transformação">
                  Observe as mudanças na paisagem e conheça o contexto regional.
                </Chapter>
                <LandCoverHistory
                  key={selected.municipality.codigo_ibge}
                  landCover={selected.land_cover}
              ...
              <section id="capitulo-preparacao">
                <Chapter number="03" title="Preparação em evidência">
                  Estruturas declaradas, saneamento e instrumentos federais:
                  cada fonte responde uma pergunta.
                </Chapter>
              ...
              <div className="elevated-card mt-5 flex flex-col justify-between gap-5 rounded-2xl border border-border bg-card p-5 shadow-sm sm:flex-row sm:items-center sm:p-6">
                <div className="max-w-2xl">
                  ...
                  <p className="mt-1 text-sm leading-6 text-muted-foreground">
                    Consulte o painel oficial de alertas ativos. Ele é separado
                    do histórico municipal apresentado acima.
                  </p>
                </div>
            ...
            <div className="rounded-2xl border border-dashed border-border bg-card/70 p-8 text-center">
              ...
              <p className="mt-2 text-sm text-muted-foreground">
                Busque sua cidade para conhecer os dados disponíveis ou explore
                o exemplo de Blumenau.
              </p>
            </div>
          )}
        </section>
          ...
          <div className="mx-auto max-w-7xl px-4 py-12 sm:px-8 sm:py-16">
            <div className="max-w-2xl">
              ...
              <p className="mt-4 text-base leading-7 text-muted-foreground">
                Cada bloco responde uma pergunta específica. Nenhum deles,
                sozinho ou somado, diz se uma cidade está “protegida”.
              </p>
            </div>
          ...
          <div className="mx-auto max-w-7xl px-4 py-12 sm:px-8 sm:py-16">
            <div className="grid gap-6 lg:grid-cols-[0.8fr_1.2fr] lg:items-end">
              <div>
                ...
                <h2 className="font-heading text-3xl font-semibold tracking-tight sm:text-4xl">
                  Cada dado tem um ponto de partida.
                </h2>
              </div>
            ...
            <div className="source-catalog">
              {[
                {
                  label: 'Histórico',
                  title: 'Atlas Digital de Desastres',
                  version: metadata?.sources.atlas.release,
                  text: metadata
                ...
                },
                {
                  ...
                  text: 'Cobertura e uso da terra municipal na série da Coleção 11. Classificação do território, sem inferência de risco ou causalidade.',
                  href: MAPBIOMAS_URL,
                  action: 'Consultar MapBiomas',
                },
                {
                ...
                <article key={source.title}>
                  <div>
                    ...
                    <h3 className="mt-1 leading-snug">{source.title}</h3>
                    {source.version && (
                      <details className="data-details">
                        <summary>Versão dos dados</summary>
                        <p className="break-all">{source.version}</p>
                      </details>
                    )}
                  </div>
        ...
        <footer className="relative overflow-hidden bg-primary text-primary-foreground">
          ...
          <div className="relative mx-auto grid max-w-7xl gap-10 px-4 py-12 text-sm sm:px-8 lg:grid-cols-[1.35fr_0.75fr_1fr_1.25fr] lg:py-14">
            <div>
              ...
              <a
                className="mt-5 inline-flex min-h-11 items-center gap-2 rounded-lg border border-white/30 px-4 font-bold hover:bg-white/10"
                href="https://github.com/rasterxtech/antes-da-chuva"
                target="_blank"
                rel="noreferrer"
              >
                Conheça o código do projeto{' '}
                <ExternalLink aria-hidden="true" className="size-4" />
              </a>
            </div>

            <div>
              <h2 className="text-sm font-bold text-white/78">Navegue</h2>
              <nav
            ...
            <div>
              ...
              <div className="mt-4 grid gap-3 text-white/82">
                ...
                {sourceLink(MAPBIOMAS_URL, 'MapBiomas Brasil', true)}
                {sourceLink(
            ...
            <div>
              <h2 className="text-sm font-bold text-white/78">Transparência</h2>
