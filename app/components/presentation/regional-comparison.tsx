...
const definitions = [
  {
    ...
    title: 'Registros relacionados à chuva nos últimos 10 anos',
    suffix: ' registros',
    change: false,
  },
  {
    key: 'urban_change_20y_pct',
    title: 'Variação da área urbanizada em 20 anos',
    suffix: '%',
    change: true,
  },
  {
    key: 'native_vegetation_change_20y_pct',
    title: 'Variação da vegetação nativa em 20 anos',
    suffix: '%',
    change: true,
  },
  {
    key: 'urban_area_pct',
    title: 'Área urbanizada atual',
    suffix: '%',
    change: false,
  },
  {
    key: 'native_vegetation_area_pct',
    title: 'Vegetação nativa atual',
    suffix: '%',
    change: false,
  },
] as const;
...
export function RegionalComparison({
  benchmarks,
}: Pick<MunicipalityPresentation, 'benchmarks'>) {
  const region = benchmarks.immediate_region;
  return (
    <section
      aria-labelledby="comparacao-regional"
      className="elevated-card mt-5 rounded-2xl border border-border bg-card p-5 shadow-[0_12px_45px_rgb(21_42_57/8%)] sm:p-7"
    >
      <p className="text-sm font-bold text-rain-strong">Contexto territorial</p>
      <h2
        className="mt-1 font-heading text-2xl font-semibold"
        id="comparacao-regional"
      >
        Como este município se compara à região?
      </h2>
      <p className="mt-2 max-w-3xl text-sm leading-6 text-muted-foreground">
        Comparações independentes com os municípios da Região Geográfica
      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-2 xl:grid-cols-3">
        {definitions.map(({ key, title, suffix, change }) => {
          ...
          const hasPercentile =
            metric.percentile_strictly_lower_pct !== null &&
            denominator.included > 1;
          const referenceDate =
            key === 'rain_related_event_count_10y'
